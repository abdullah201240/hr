...
export class SalaryService {
    ...
    const result = await Promise.all(
      templates.map(async (template) => {
        const components = await this.db
          ...
          .where(eq(salaryTemplateComponents.templateId, template.id))
  ...
  async createTemplate(dto: CreateSalaryTemplateDto) {
    ...
    return this.findOneTemplate(created.id);
  }
  ...
  async findAllEmployeeSalaries() {
    const cached = await this.cache.getByKey(CacheKeys.employeeSalaryList);
    if (cached) return cached;
    ...
    await this.cache.setByKey(CacheKeys.employeeSalaryList, result);
    return result;
  }
