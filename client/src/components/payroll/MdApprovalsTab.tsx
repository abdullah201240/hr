...
export function MdApprovalsTab({
  selectedMonth,
  setSelectedMonth,
  cycle,
  formatCurrency,
  ...
  // Filter payslips that are in Awaiting_MD_Approval status
  const eligiblePayslips = useMemo(() => {
    return payslipsList.filter((p) => p.status === "Awaiting_MD_Approval")
  }, [payslipsList])
  ...
  const handleBulkApprove = () => {
    Swal.fire({
      ...
      text: `Are you sure you want to approve all pending MD-level payslips for ${selectedMonth}?`,
      icon: "question",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        bulkApproveMutation.mutate(selectedMonth, {
          onSuccess: (res) => {
            Swal.fire({
              title: "Bulk Approved!",
              text: res.message || "All MD payslips approved successfully.",
              icon: "success",
              confirmButtonText: "Close",
              buttonsStyling: false,
              customClass: {
      ...
      <CardHeader className="pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <CardDescription className="text-xs">
            Review and finalize company payroll cycles before accounts release.
          </CardDescription>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {eligiblePayslips.length > 0 && hasPermission("payroll:approve_md") && (
            <Button
              size="sm"
              ...
              {eligiblePayslips.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12 text-muted-foreground text-xs">
                    ...
                    No pending payslips require Managing Director approval for this month.
                  </TableCell>
                </TableRow>
              ) : (
